...
```{r, eval = FALSE}
new_master8 <- cbind("Pred" = new_master7$First_Prediction, new_master5)
new_master8$Pred <- as.factor(new_master8$Pred)
set.seed(123)
cv = vfold_cv(new_master8, v = 5)

grid <-expand_grid(neighbors = c(1:9))
...
results_1<-tune_grid(spec, Pred ~ ., grid = grid, resamples = cv)
```

```{r}
m3 <- Mclust(scaled_new_master5, G = 6)
scaled_new_master5 <- scale(new_master5)
new_master9 <- cbind(new_master6,data.frame(new_master5))
new_master9$new_model <- m3$classification
d3 <- purrr::map(1:6, ~ new_master9%>%select_if(is.numeric) %>% filter(new_model== {{.x}}))
pred_3_median<-map_df(d3, ~ .x %>% summarise(across(everything(), median)))
pred_3_mean<-map_df(d3, ~ .x %>% summarise(across(everything(), mean)))
new_master9%>%filter(new_model == 3)
```
