...
as we could. Figure 1, created from the scifigure package
[@Patil], illustrates in what areas our replication is different from
...
classify as a full reproduction of @kalman_nba_2020. In Figure 1,
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, eval = TRUE, fig.cap="Reproduction vs. Replication", fig.topcaption = TRUE}
library(scifigure)
exps <-
  init_experiments(
    nexp = 3,
...
being able to play efficiently for 35 minutes). Table 1
displays all of the variables included as well as a short description
about each variable.
...
will be in each cluster [@Maklin]. Figure 2 [@Carrasco] displays
...
```{r, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Gaussian Mixture Modeling", fig.topcaption = TRUE}
set.seed(1)
df <- tibble(
  r = c(rnorm(10, -5, 1),
        rnorm(10, 2, 0.5),
...
in the same cluster in both models). Table 2 displays the six
...
significantly. Figure 3 shows how the proportion of our
clusters compared to theirs.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Proportion Analysis", fig.topcaption = TRUE}
library(reshape2)
...
ggplot(proportion_comparison, aes(x = First_Prediction, y = value, fill = variable%>%relevel("Kalman and Bosch Proportions")))+
  ...
  labs(
    ...
    title = "Distribution of Players Across 9 Clusters",
    fill = "Data Used"
  ) +
...
section. Figure 4 shows the proportions of players in each cluster in both the new and old seasons. 

```{r, echo=FALSE, fig.cap= "New vs. Old Proportion Analysis", fig.topcaption = TRUE}
...
cluster. Figure 5 is a graphical representation of the Traditional Center
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, fig.cap= "Traditional Center Cluster", fig.topcaption = TRUE}
Trad_Cent <-
...
teams' evolution over the course of the 13 year period. Figure 6 displays a heat map of how the Golden State Warriors have gone about
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Golden State Warriors", fig.topcaption = TRUE}
...
worth of data. Figure 7 displays a heat map that uses the probabilities that a player was in
each cluster. For example it accounts for the 25% that a player is in
one cluster even though they are primarily classified in a different
cluster. As seen in Figure 7, the Traditional Center cluster
...
```{r, echo = FALSE, fig.cap= "League Trends", fig.topcaption = TRUE}
...
Figure 8 displays the proportion of players in each cluster
...
```{r, echo = FALSE, fig.cap= "Variable Reduction", fig.topcaption = TRUE}
...
Table 3 shows the percentage of players for each cluster in the
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.topcaption = TRUE}
library(knitr)
...
models_1_4%>%
  ...
  kable(caption = "Cluster Consistency")
...
Dominant Scorer, Traditional Center, and Versatile Role Player. Table 4 outlines each of the clusters and the strengths and
...
|                 | Field Goal %.           |              |              |
+-----------------+-------------------------+--------------+--------------+
| Complimentary   | Secondary creator to    | Assist Rate  | Off. Reb %   |
| Guard           | the Ball Dominant       |              |              |
|                 | Scorer. Has the highest | 3P FG%       | Height       |
|                 | assist rate among the   |              |              |
|                 | clusters and a strong 3 |              |              |
|                 | point shooting          |              |              |
|                 | percentage, though this |              |              |
|                 | cluster doesn't take    |              |              |
|                 | many 3s. Weak in height |              |              |
|                 | and defensive           |              |              |
|                 | rebounding.             |              |              |
+-----------------+-------------------------+--------------+--------------+
| Three Point     | A guard or wing who     | 3P FG%       | Off. Reb %   |
| Shooter         | takes a lot of 3's and  |              |              |
|                 | is highly efficient     | 3P FGA       | FT Rate      |
|                 | from 3. Has a very low  |              |              |
|                 | FT rate and does not    |              |              |
|                 | crash the offensive     |              |              |
|                 | glass.                  |              |              |
+-----------------+-------------------------+--------------+--------------+
...
|                 | players.                |              |              |
+-----------------+-------------------------+--------------+--------------+
| Traditional     | Interior post center    | Off. Reb %   | 3p FG%       |
| Center          | who shoots almost       |              |              |
...
+-----------------+-------------------------+--------------+--------------+
| Versatile Role  | Group of players who    | Assist Rate  | Off. Reb %   |
| Player          | act as chameleons on    |              |              |
|                 | the court and can do    | Usage Rate   | FGA (0-10    |
...
Figure 9 shows how players from the original nine clusters fit
...
```{r, message = FALSE, echo=FALSE, warning=FALSE, fig.cap= "Cluster Reduction", fig.topcaption = TRUE}
...
of the players remained in the same cluster. This is remarkably similar
to the
`r round(sum(reduced_preds%>%summarize(First_Prediction == New_Prediction))/nrow(reduced_preds)*100, 2)`%
of the players that remained in the same cluster in the reduced variable
model with nine clusters. We believe that this further demonstrates that
the reduction in variables is the main driver in why these two pairs of
models have a significantly lower cluster consistency than the original
model and its pair.
