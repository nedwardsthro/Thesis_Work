...
data and the analysis outline, another analyst could reproduce the same
...
[@Plesser_2018] An example of reproduction of Kalman and Bosch's
analysis would be to use a new population (perhaps NCAA basketball data)
and to arrive at the same nine clusters that they arrived at.

Our paper is an example of a replication of the analysis that Kalman and
Bosch performed. To replicate their analysis, we endeavored to use as
close to the same data and methods that Kalman and Bosch used as we
could. The graphic below illustrates in what area our replication is
different from their analysis and in what areas it is the same. Finally,
it shows how our expansion is different from both the original analysis
and our replication. In the figure, black icon mean that trait of the
analysis is the same as the original while blue traits are different
...
While we desired to stay as close to the process that Kalman and Bosch
...
website that Kalman and Bosch manually scraped their data from). Using
...
scraped by Kalman and Bosch. Additionally, the shot distribution
...
Kalman and Bosch's analysis [@kalman_nba_2020]. Through some
exploration, we found that the `r 5512 - nrow(master)` discrepancy in
observations has to do with players who are traded. In our data set, any
...
with 3,608 observations [@kalman_nba_2020]. After using the same filter,
we ended with `r nrow(scaled_master_data)` observations, leading to a
...
We used the same variables that Kalman and Bosch used in their model
...
minute statistics will lead to slight differences in scale versus using
per 100 possession statistics but the important thing is that both are
rate statistics.\
...
+----------------+------------------------------------------+
| Steal Rate     | \% of opponent possessions that end with |
|                | a steal b the player while on the floor  |
+----------------+------------------------------------------+
...
| Rating         |                                          |
+----------------+------------------------------------------+
| Free Throw     | Number of free throws made per field     |
| Rate           | goals attempted                          |
+----------------+------------------------------------------+
| Free Throw     | Number of free throws made per free      |
| Percentage     | throw attempt                            |
+----------------+------------------------------------------+
...
|                | attempt                                  |
+----------------+------------------------------------------+
| 3FG%           | Number of three-point field goals made   |
|                | per attempt                              |
+----------------+------------------------------------------+
...
+----------------+------------------------------------------+
| Corner 3FGA%   | \% of three point-field goal attempts    |
|                | that are from the corner                 |
+----------------+------------------------------------------+
...
| Rate           | dunks                                    |
+----------------+------------------------------------------+
| 0-3 ft FGA%    | \% of all field goal attempts between    |
|                | zero and three feet from the basket      |
+----------------+------------------------------------------+
| 3-10 ft FGA%   | \% of all field goal attempts between    |
|                | three and ten feet from the basket       |
+----------------+------------------------------------------+
| 10ft-3p FGA%   | \% of all field goal attempts between    |
|                | ten feet from the basket and the         |
|                | three-point line                         |
+----------------+------------------------------------------+

: Recreation of Table 1 from Kalman and Bosch [@kalman_nba_2020]
...
oblong shape. An advantage of model-based clustering is that it assigns
"soft clusters", showing the probability that each observation will be
in each cluster [@Maklin]. The figure below [@Carrasco] displays a
two-dimensional graphical representation of the clustering distributions
produced by Gaussian mixture modeling. As you can see, each cluster has
not only a different mean, but a different variance.

![](images/Gaussian%20Mixture%20Modeling.png)

As done by Kalman and Bosch, we used the `mclust` [@fraley2006] package
in R to implement the Gaussian mixture clustering.
...
2009-2018 seasons, we wanted to see what predictions the model would
...
the accuracy of a Gaussian Mixture model. There is not necessarily a
"correct" way to cluster the observations and in many ways, having a
good model is based on expert knowledge of the data and being satisfied
with the results. In an effort to come up with some test metric, we
decided to test how the clustering predictions hold for each pair from
one model to the other (i.e. does a player classify in the same cluster
in both models). Below is a table displaying the six models (three
...
wanted to see how this proportion matched with that of Kalman and
Bosch's analysis. Following this, we took a deep dive into each cluster
to make sure that our cluster characteristics matched that of Kalman and
Bosch. While Kalman and Bosch showed the Three Point Shooting Cluster in
their analysis, we opted to show the Traditional Center cluster to give
readers a view of a different cluster. Following this, we investigated
if the the clustering method could accurately capture the evolution of
individual teams and the league as a whole. Next we analyzed cluster
consistency between our model with only ten seasons and our model with
all 13 seasons.
...
seasons that Kalman and Bosch used, the three new seasons that we added
...
displays the a heat map of how the Warriors have gone about team
building the past 13 years. We picked the Golden State Warriors to show
because they historically have been averse to making mid-season trades,
meaning that all of their players will show up as playing for their team
and will not have a "TOT" team name in our data set. The most noticeable
part of this figure is how the Warriors moved from Traditional Centers
to Mid Range Bigs. While they are most recently known for their "Death
Lineup" and three point shooting, it is not that the Warriors don't have
big men, but rather that they have transitioned from Traditional Centers
to Mid Range Bigs. The Andrew Bogut/Festus Ezeli pairing that marked the
early 2010's has been replaced by the more mid range oriented Kevon
Looney/Eric Pascall pairing. The three year period of Kevin Durant can
also be seen by an increase in the Ball Dominant Scorer cluster.
Finally, it is interesting to note that the Warriors have more Versatile
Role Players than most teams with players such as Draymond Green and
Shaun Livingston often being classified as Versatile Role Players.
...
of data. This heat map uses the probabilities that a player was in each
cluster so it accounts for the 25% that a player is in one cluster even
though they are primarily classified in a different cluster. As seen in
the heat map below, the Traditional Center cluster had the highest
proportion at the beginning of this time frame but gradually decreased
as the NBA moved towards a more perimeter oriented game. At the same
time, the Three Point Shooting Guard cluster gradually increases before
exploding over the last five years where now over 25% of the players in
the most recent completed season classify as a Three Point Shooting
Guard. The transition away from a game dominated in the paint can also
be demonstrated by slight increases in the proportions of the Stretch
Forward, Mid Range Big, and Skilled Forward clusters. Finally, we
thought it was particularly interesting that in the last few years, the
Ball Dominant Scorer cluster has increased with slight declines in the
Floor General and High Usage Guard clusters.
...
While working with the data from the Kalman and Bosch analysis, we
...
of models. We hypothesize that this lower cluster consistency is due to
...
+=============+=============+=============+=============+
| Skilled Big | A big who   | Def Reb. %  | 3P FGA      |
|             | does plays  |             |             |
|             | both inside | 2P FG %     |             |
|             | and outside |             |             |
|             | of the      |             |             |
...
|             | %.          |             |             |
+-------------+-------------+-------------+-------------+
| Co          | Secondary   | Assist Rate | Off. Reb %  |
| mplimentary | creator to  |             |             |
| Guard       | the High    | 3P FG%      | Height      |
|             | Usage       |             |             |
|             | Scorer. Has |             |             |
|             | the highest |             |             |
|             | assist rate |             |             |
...
|             | rebounding. |             |             |
+-------------+-------------+-------------+-------------+
| Three Point | A guard or  | 3P FG%      | Off. Reb %  |
| Shooter     | wing who    |             |             |
|             | takes a lot | 3P FGA      | FT Rate     |
|             | of 3's and  |             |             |
|             | is highly   |             |             |
|             | efficient   |             |             |
|             | from 3. Has |             |             |
...
|             | glass.      |             |             |
+-------------+-------------+-------------+-------------+
| Ball        | A high      | FGA (10     | \% of FGA   |
| Dominant    | usage       | FT-3P)      | from 3      |
| Scorer      | player,     |             |             |
|             | typically a | Point Per   |             |
|             | guard or    | Minute      |             |
|             | wing, who   |             |             |
|             | is          |             |             |
|             | proficient  |             |             |
...
|             | players.    |             |             |
+-------------+-------------+-------------+-------------+
| Traditional | Interior    | Off. Reb %  | 3p FG%      |
| Center      | post center |             |             |
|             | who shoots  | FT Rate     | Assist Rate |
|             | almost      |             |             |
|             | exclusively |             |             |
|             | inside ten  |             |             |
|             | feet. High  |             |             |
...
|             | rate.       |             |             |
+-------------+-------------+-------------+-------------+
| Versatile   | Group of    | Assist Rate | Off. Reb %  |
| Role Player | players who |             |             |
|             | act as      | Usage Rate  | FGA (0-10   |
|             | chameleons  |             | FT)         |
|             | on the      |             |             |
|             | court and   |             |             |
|             | can do      |             |             |
...
can debate whether or not we would classify these players as High Usage
Scorers based on expert knowledge but it is not surprising that they are
classified as such with a reduction in clusters. The Complimentary Guard
cluster is mostly a collection of Floor Generals, Three Point Shooting
Guards, and High Usage Guards, about what we expected it would be. The
Skilled Big cluster too is largely what we thought it would be - a mix
of Mid Range Bigs, Skilled Forwards, and a few Traditional Centers. The
Three Point Shooter cluster is about what we predicted it would be,
though more Mid Range Bigs ended up being classified in this cluster
than we hypothesized. This suggests that a better name for this cluster
might be "Floor Spacer" rather than "Three Point Shooter". The
Traditional Center cluster is made up almost entirely of Traditional
Centers from the original model, again demonstrating a strong cluster
identity for Traditional Centers. Finally, the Versatile Role Player
cluster retained the majority of the Versatile Role Players from the
original model and added at least a few of players from almost every
other cluster.
...
40% of a player that is in one cluster even though they are primarily
listed in another cluster.
...
that Kalman and Bosch used and a second model based on 13 seasons worth
...
not want to corner players only into a single cluster (the whole point
of "soft" clusters with Gaussian mixture modeling), but it is important
to identify that there are real differences in skill sets between
positions.
...
Luka Doncic, and Trae Young has changed the league. These starcentric
offenses developed in an effort to be more efficient and we believe that
over time, this trend will only continue to grow. The NBA has taken some
measures to reduce the efficiency of this style of play, such as
changing some of the rules relating to fouling that Harden and Young
have so famously taken advantage of. Despite the NBA's effort, we
believe that the league as a whole (especially as it relates to offense)
will continue to gradually grow more starcentric. Further research
should be done in the future to investigate this trend and potential
ceilings on this trend.

# 5. Conclusion

We concur with Kalman and Bosch that NBA players can be clustered
...
they are better clustered into the original nine clusters that Kalman
and Bosch used. However, as the NBA continues to evolve, we believe that
a reduction in clusters would be optimal.
...
work begun by Kalman and Bosch and that others will expand on our work
